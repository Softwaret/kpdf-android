...
internal class MockApiServiceImpl : ApiService {
    ...
    private val publications = mutableSetOf(
        Publication(
            1, "Sample", "Jan Mock",
            ...
            "Opisik naszego przykładowego pdfka, niezbyt długi i niezby krótki taki raczej w sam raz."
        )
    )
    private val users = mutableSetOf(
        User("test", "test", "test name")
    )
    ...
    override suspend fun loginUser(login: String, password: String) =
        if (users.any { it.login == login && it.password == password }) {
            LoginResponse(TOKEN, REFRESH_TOKEN).asValue()
        } else {
            Try.Error
        }

    override suspend fun refreshToken(login: String, refreshToken: String) =
        if (refreshToken == REFRESH_TOKEN) {
            RefreshTokenResponse(TOKEN, REFRESH_TOKEN).asValue()
        } else {
            Try.Error
        }

    override suspend fun getPublications() =
        GetPublicationsResponse(publications.map { it.id })
            .asValue()
